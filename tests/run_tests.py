...
Version: 1.3
Timestamp: 2025-03-01 21:47 CET
...
class TestRunner:
    ...
    def run_integration_tests(self) -> None:
        ...
        import tests.integration.test_websocket_multiple_connections
        ...
        from tests.integration.test_text_processing import (
            run_basic_tests,
            run_edge_case_tests,
        ...
        self.run_test(
            "Text Buffer",
            lambda: unittest.TextTestRunner().run(
                unittest.defaultTestLoader.loadTestsFromModule(tests.integration.test_text_buffer)
            ),
        )
        self.run_test(
            "Tumbling Window",
            lambda: unittest.TextTestRunner().run(
                unittest.defaultTestLoader.loadTestsFromModule(
                    tests.integration.test_tumbling_window
                )
            ),
        )
        self.run_test(
            "Tumbling Window WebSocket Integration",
            lambda: unittest.TextTestRunner().run(
                unittest.defaultTestLoader.loadTestsFromModule(
                    tests.integration.test_tumbling_window_websocket
                )
            ),
        )
        self.run_test(
            ...
            lambda: unittest.TextTestRunner().run(
                unittest.defaultTestLoader.loadTestsFromModule(
                    tests.integration.test_websocket_state_tracking
                )
            ),
        )
        self.run_test(
            ...
            lambda: unittest.TextTestRunner().run(
                unittest.defaultTestLoader.loadTestsFromModule(
                    tests.integration.test_websocket_multiple_connections
                )
            ),
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "category", choices=["timing", "integration", "speech", "all"], help="Test category to run"
    )

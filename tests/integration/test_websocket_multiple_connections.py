...
class WebSocketMultipleConnectionsTest(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        for instance in self.test_instances:
            try:
                if hasattr(instance, 'state') and instance.state != ConnectionState.DISCONNECTED:
                    print(f"DEBUG: Cleaning up instance {instance.client_id} in state {instance.state.name}")
                    instance.cleanup()
                    
                    # Verify cleanup was successful
                    if instance.state != ConnectionState.DISCONNECTED:
                        print(f"WARNING: Instance {instance.client_id} not properly disconnected, forcing state change")
        ...
        with WhisperWebSocket._instances_lock:
            ...
            print(f"DEBUG: Cleared active instances, count: {len(WhisperWebSocket._active_instances)}")
    ...
    def test_client_and_session_ids(self):
        ...
        self.assertEqual(ws_client.state, ConnectionState.READY,
                        "Client should be in READY state after connect")
        ...
        self.assertNotEqual(ws_client.session_id, original_session_id, 
                           "Session ID should change after reconnect")
    ...
    def test_instance_tracking(self):
        ...
        self.assertEqual(WhisperWebSocket.get_instance_count(), 0,
        ...
        self.assertEqual(WhisperWebSocket.get_instance_count(), 1,
        ...
        self.assertEqual(WhisperWebSocket.get_instance_count(), 2,
        ...
        # Verify client is in DISCONNECTED state after cleanup
        print("DEBUG: Verifying client is in DISCONNECTED state after cleanup")
        self.assertEqual(ws_client1.state, ConnectionState.DISCONNECTED,
                        "Client should be in DISCONNECTED state after cleanup")
        ...
        # Verify client is in DISCONNECTED state after cleanup
        print("DEBUG: Verifying client is in DISCONNECTED state after cleanup")
        self.assertEqual(ws_client2.state, ConnectionState.DISCONNECTED,
                        "Client should be in DISCONNECTED state after cleanup")
    ...
    def test_cleanup_all_instances(self):
        ...
        for client in ws_clients:
            self.assertEqual(client.state, ConnectionState.READY,
                            "All clients should be in READY state")
        ...
        with WhisperWebSocket._instances_lock:
            instance_count = len(WhisperWebSocket._active_instances)
            if instance_count > 0:
                print(f"WARNING: {instance_count} instances still active after cleanup_all_instances")
                for client_id, instance in list(WhisperWebSocket._active_instances.items()):
    ...
    def test_connection_throttling(self):
        ...
        if current_time - ws_client.last_connection_attempt < config.WS_RECONNECT_DELAY:
            wait_time = config.WS_RECONNECT_DELAY - (current_time - ws_client.last_connection_attempt)
        ...
        self.assertAlmostEqual(args[0], config.WS_RECONNECT_DELAY, delta=0.1,
    ...
    def test_reconnection_with_new_session(self):
        ...
        self.assertNotEqual(ws_client.session_id, original_session_id,
                           "Session ID should change after reconnection")

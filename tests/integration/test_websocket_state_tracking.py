...
class WebSocketStateTrackingTest(unittest.TestCase):
    ...
    def test_thread_safety(self):
        ...
        # Verify final state
        self.assertEqual(
            self.ws_client.state, ConnectionState.CLOSED, "Final state should be CLOSED"
        )
...
class WebSocketMultipleConnectionsTest(unittest.TestCase):
    ...
    def test_multiple_connections(self):
        ...
        self.assertEqual(ws_client1.state, ConnectionState.READY, "First client should be ready")

        # Connect second client
        ws_client2.connect()

        # Verify second client is ready
        self.assertEqual(ws_client2.state, ConnectionState.READY, "Second client should be ready")

        # Verify client IDs are different
        self.assertNotEqual(
            ws_client1.client_id, ws_client2.client_id, "Each client should have a unique client ID"
        )

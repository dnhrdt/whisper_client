    ...
    def run_test(self, name, segments, expected_outputs=None, reset=True, delay=0.2):
        ...
        if expected_outputs is not None:
            if len(actual_outputs) != len(expected_outputs):
                result["passed"] = False
                result["details"].append(f"Expected {len(expected_outputs)} outputs, got {len(actual_outputs)}")
            
            for i, (actual, expected) in enumerate(zip(actual_outputs, expected_outputs)):
                if actual != expected:
                    result["passed"] = False
                    result["details"].append(f"Output {i+1} mismatch: Expected '{expected}', got '{actual}'")
